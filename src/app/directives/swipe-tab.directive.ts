...
export class SwipeTabDirective implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        const forumsTabsList = this._el.nativeElement.querySelectorAll('ion-tab-button');

        for (let i = 0, len = forumsTabsList.length; i < len; i += 1) {
            this.tabNames.push(forumsTabsList[i].tab);
        }
